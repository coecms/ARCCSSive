...
def compare_instances(db,remote,local,const_keys,admin):
    ...
    # update local version on database
            if admin: 
               ...
               if db.dirty:
                 line=["version"]+list(ds_instance.values())+[str(v.id),v.version,v.dataset_id,str(v.is_latest),v.checked_on,"\n"]
                 #line+=list(ds_instance.valuess())+[str(v.id),v.version,v.dataset_id,str(v.is_latest),v.checked_on,"\n"]
